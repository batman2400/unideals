...
export async function uploadBrandLogo({ file, brandName }) {
  if (!file) {
  ...
  const filePath = `logos/${brandSegment}-${Date.now()}-${randomSuffix}.${extension}`;
  ...
  const {
    data: { publicUrl },
  } = supabase.storage.from(BRAND_LOGOS_BUCKET).getPublicUrl(filePath);
